...
public class RestAPIExample {
    private Map<String, Twitter.User> userProfile = new HashMap<>();
    private Map<String,List<Twitter.Tweet>> tweets = new HashMap<>();
    private Map<String,List<String>> following = new HashMap<>();
    private Twitter.UserDao userDao = new Twitter.UserDaoImpl();
    private Twitter.TweetDao tweetDao = new Twitter.TweetDaoImpl();
    private Twitter.FollowDao followDao = new Twitter.FollowDaoImpl();

    public RestAPIExample() {
        List<Twitter.User> users = userDao.readAll();
        for (Twitter.User user : users) {
        ...
        for (int i = 0 ; i < list.size() ; i++){
            ...
            Twitter.Tweet tweet = new Twitter.Tweet(arr[0].toString(), arr[1].toString(), arr[2].toString(),(LocalDateTime)arr[3]);
            String email = arr[1].toString();
            if(tweets.containsKey(email)){
                tweets.get(email).add(tweet);
            }
            else {
                List<Twitter.Tweet> tweetList = new ArrayList<>();
                tweetList.add(tweet);
                tweets.put(arr[1].toString(),tweetList);
            }
        }

        List<Object[]> followList = followDao.following();
        for (int i = 0 ; i < followList.size() ; i++){
            Object[] array = followList.get(i);
            String email = array[0].toString();
            String userEmail = array[1].toString();
            if (following.containsKey(email)) {
                following.get(email).add(userEmail);
            } else {
                List<String> list2 = new ArrayList<>();
                list2.add(userEmail);
                following.put(email, list2);
            }
        }
    }
    ...
    public ModelAndView getUserDetails() {
        ...
        if (userProfile.isEmpty())
            allAccDetails();
        List<Twitter.User> users = new ArrayList<>();
        for(Map.Entry entry: userProfile.entrySet()){
            users.add((Twitter.User)entry.getValue());
        }
    ...
    public ModelAndView fetchTweets(@RequestParam String email) {
        ...
        List<Twitter.Tweet> tweetList = tweetDao.readByEmail(email);
    ...
    public ModelAndView signUp(@RequestBody MultiValueMap<String, String> formData) {
        ...
        Twitter.User user = new Twitter.User(name,email,password);
    ...
    public ModelAndView login(@RequestBody MultiValueMap<String, String> formData) throws Twitter.UserNotRegistered {
        if(userProfile.isEmpty())
            allAccDetails();
        if (!isUserRegistered(formData)) {
            throw new Twitter.UserNotRegistered();
    ...
    private boolean isUserValid(MultiValueMap<String, String> map) {
        ...
        Twitter.User user = userProfile.get(email);
    ...
    @PostMapping("/create")
    private ResponseEntity<String> createUser(@RequestBody Twitter.User user) {
    ...
    Map<String, Twitter.User> allAccDetails() {
        return userProfile;
    }

    //    User can fetch particular account details --> GET
    @GetMapping("/getDetails")
    private ResponseEntity<Twitter.User> getAccDetails(@RequestParam String email, String password) {
        ResponseEntity<Twitter.User> responseEntity = null;
        if(userProfile.containsKey(email)){
    ...
    @PutMapping("/update")
    private ResponseEntity<String> updateRecord(@RequestBody Twitter.User user) {
        String email =user.getEmail();
    ...
    ModelAndView createTweet(@RequestBody MultiValueMap<String, String> newTweet){
        ...
        String name = userProfile.get(email).getName();
        Twitter.Tweet t = new Twitter.Tweet(name,email,content,time);
        if (userProfile.isEmpty())
            allAccDetails();
        if(userProfile.containsKey(email)){
                if(tweets.containsKey(email))
                    ...
                    List<Twitter.Tweet> list = new ArrayList<>();
                    list.add(t);
    ...
    Map<String,List<Twitter.Tweet>> fetchTweets(){
            return tweets;
    }

    //user can see tweets from a particular account
    @GetMapping("/fetchTweetsOfUser")
    List<Twitter.Tweet> fetchTweetsOfUser(@RequestParam String email){
        return tweets.get(email);
    }

    //user can follow another user
    @PostMapping(value = "/followUser",consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE)
    private ModelAndView followUsers(@RequestBody MultiValueMap<String,String> emails){
        //ResponseEntity<String> responseEntity = null;
        ...
        if (userProfile.get(userEmail) != null) {
            ...
            Twitter.Follow f = new Twitter.Follow(email,userEmail);
            followDao.follow(f);
            ...
            return errorMessageModelAndView("User you are following doesn't exist");
        }
    }

    //to view the users followed
    ...
    @GetMapping("/follow")
    private List<String> follow(@RequestParam String email){
        if(following.containsKey(email))
